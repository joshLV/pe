 ...
 * @since 1.0
 */
public class StepBatchTrsTemplate extends AbstractApplicationObjectTemplate {
	...
	public void execute(Context context) throws PeException {
		...
		if ((prePhase.equals(clearingEntity.getClearPhase()) && clearingEntity.getClearStatus().equals(ClearingEntity.CLEAR_STATUS_OK))
			...
			while (true) {
				retry++;
				log.info(getApplicationContext().getMessage("batch_step_" + currentPhase, new Object[] { departmentEntity.getDepartmentName() }, Locale.getDefault())
						+ getApplicationContext().getMessage("batch_proc_begin", new Object[] { new Integer(retry) }, Locale.getDefault()));
				...
				try {
					...
					log.info(getApplicationContext().getMessage("batch_step_" + currentPhase, new Object[] { departmentEntity.getDepartmentName() }, Locale.getDefault())
							+ getApplicationContext().getMessage("batch_proc_success", new Object[] { new Integer(retry) }, Locale.getDefault()));
					break;
				} catch (Exception ex) {
					log.error(getApplicationContext().getMessage("batch_step_" + currentPhase, new Object[] { departmentEntity.getDepartmentName() }, Locale.getDefault())
							+ getApplicationContext().getMessage("batch_proc_error", new Object[] { new Integer(retry) }, Locale.getDefault()), ex);
					...
					if (retry >= retryTimes) {
						...
						throw new PeException(getApplicationContext().getMessage("batch_step_" + currentPhase, new Object[] { departmentEntity.getDepartmentName() }, Locale.getDefault())
								+ getApplicationContext().getMessage("batch_proc_error", new Object[] { new Integer(retry) },
