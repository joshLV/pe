 ...
 * 准备申请退款订单
 */
public class PrepareRefundForMerOrderAction extends AbstractAction {
	...
	public void execute(Context ctx) throws PeException {
		String outRefundNo = MiscUtil.toStringAndTrim(ctx.getData("merseqno"));
		String merDate =MiscUtil.toStringAndTrim(ctx.getData("merdate")).substring(0,8);
		Order order = null;
		order = orderManager.getOrderBySeqNoMerDate(outRefundNo,merDate);
		if(order == null){//新建退货订单
			order = new Order();
			// 流水号
			String transSeqNo = refundOrderSeqNoGenerator.generate().toString();
			order.setTransSeqNo(transSeqNo);
			order.setMerSeqNo(outRefundNo);
			//商户日期
			String merchantDate =
