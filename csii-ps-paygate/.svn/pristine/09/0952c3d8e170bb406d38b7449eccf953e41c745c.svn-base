...
public class WP01Action extends AbstractAction {
	...
	public void execute(Context ctx) throws PeException {
		...
		sendMap.put("appid", order.getAppid()); //银行appid
		sendMap.put("mch_id", MerBankParameterMap.get("merId"));//银行merid
		sendMap.put("sub_mch_id", order.getMerWxId());//子商户merid
			...
			if (Constants.QRCODE_SUCCESS.equals(wxResMap.get("return_code"))) {
				if (Constants.QRCODE_SUCCESS.equals(wxResMap.get("result_code"))) {
					...
					if ("USERPAYING".equals(wxResMap.get("err_code"))) {
						try {
							Thread.sleep(3000);
							Map resutlt1 = qureyUerPayingOrder(order,paysignkey,MerBankParameterMap);
							if (Constants.QRCODE_SUCCESS.equals(resutlt1.get("return_code")) && Constants.QRCODE_SUCCESS.equals(resutlt1.get("result_code"))
								...
								Thread.sleep(5000);
								Map resutlt2 = qureyUerPayingOrder(order,paysignkey,MerBankParameterMap);
								if (Constants.QRCODE_SUCCESS.equals(resutlt2.get("return_code")) && Constants.QRCODE_SUCCESS.equals(resutlt2.get("result_code"))
									...
									Thread.sleep(8000);
									Map resutlt3 = qureyUerPayingOrder(order,paysignkey,MerBankParameterMap);
	...
	private Map qureyUerPayingOrder(Order order,String paysignkey,Map merBankParameterMap) throws PeException {
		Map sendMap = new HashMap();
