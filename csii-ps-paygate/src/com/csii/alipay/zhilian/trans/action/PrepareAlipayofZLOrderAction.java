 ...
 *@create time 时间
 */
public class PrepareAlipayofZLOrderAction extends AbstractAction {
	...
	public void execute(Context ctx) throws PeException {
		...
		}else if("AGZH".equals(order.getTransId())){
			order.setPayType("JSAPI");//门店二维码H5支付
		}else if("APCP".equals(order.getTransId())) {
			order.setPayType("NATIVE");//扫码
		}
		//设置交易类型
		...
		//备注1
		order.setRemark1(MiscUtil.toStringAndTrim(ctx.getString("remark1")).equals("")?ctx.getString("remark1"):merInfo.getMerShortName() );
		//备注2
		...
		//支付宝 给分配的银行APPID  by wzj
		order.setAppid(MiscUtil.toStringAndTrim(MerBankParameterMap.get("alipayAppId")));
		//获取软证书
		...
		String rate =MiscUtil.toStringAndTrim(passageway.get("rate"));
		if(MiscUtil.isNullOrEmpty(rate)) {
			log.info("该银行ID["+order.getBankId()+"],交易类型为["+order.getPayType()+"],没有配置相关费率,请开通该服务");
			throw new PeException("E00034");
		}
		
		//计算通道手续费
		BigDecimal costFeeAmount = order.getAmount().multiply(new BigDecimal(rate));
