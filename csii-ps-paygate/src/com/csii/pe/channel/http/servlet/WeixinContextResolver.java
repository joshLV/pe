 ...
 * @since 1.0
 */
public class WeixinContextResolver extends ApplicationObjectSupport implements ContextResolver {

	private static Logger logger = LoggerFactory.getLogger(WeixinContextResolver.class);
	...
	protected void resolveContext(Context context, HttpServletRequest request, Locale locale) throws PeException {
		...
		parseJson(context, data, request, locale);
	...
	private void parseJson(Context context, byte[] data, HttpServletRequest request, Locale locale) throws PeException {
		...
		try {
			...
			if (checkTransId(reqJsonMap)) {
				...
				logger.debug("=========[{}:REQUEST JSON PARAM BEGIN]=========", transId);
				for (Map.Entry entry : reqJsonMap.entrySet()) {
					logger.debug("==>> {}:{}", entry.getKey(), entry.getValue());
				}
				logger.debug("=========[{}:REQUEST JSON PARAM END]=========", transId);
				logger.debug("=========[{}:REMOTE IP：{}]=========", transId, getRemoteIP(request));

				context.setDataMap(reqJsonMap);
				context.setData("spbill_create_ip", getRemoteIP(request));
			}
		} catch (Exception e) {
	...
	private void parse(Context context, byte[] data, HttpServletRequest request, Locale locale) throws PeException {
		...
		logger.info("REQUEST MAP:" + requestData);
		if (requestData == null) {
			throw new PeException("报文转换map为空");
		}

		context.setDataMap(requestData);
		String transId = (String) requestData.get("trans_id");
		context.setTransactionId(transId);
