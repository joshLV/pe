 ...
 * @since 1.0
 */
public class MybankNotifyContextResolver extends ApplicationObjectSupport implements ContextResolver {

	private static Logger logger = LoggerFactory.getLogger(MybankNotifyContextResolver.class);
	...
	private void parse(Context context, byte[] data, HttpServletRequest request, Locale locale) throws PeException {
		...
		logger.info("网商银行支付结果通知数据:" + requestData);

		if (requestData == null) {
			throw new PeException("报文转换map为空");
		}

		context.setTransactionId("MBNT");

		context.setData("MybankNotifyMap", requestData);
		context.setData("Message", message);
	}
